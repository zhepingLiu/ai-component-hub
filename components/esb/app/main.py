...
setup_logging("esb", LOG_DIR, LOG_LEVEL, LOG_RETENTION_DAYS, system_code=SYSTEM_CODE, max_bytes=LOG_MAX_BYTES)
...
async def esb_download(req: DownloadReq):
    ...
    async def _stream() -> AsyncIterator[bytes]:
        try:
            ...
            logger.error({"event": "esb.download.failed", "url": url, "error": str(e)}, extra=outbound_extra())
...
async def esb_upload(req: UploadReq):
    ...
    start_tm = int(datetime.now().timestamp() * 1000)
    ...
    try:
        ...
        logger.error({"event": "esb.upload.failed", "url": url, "error": str(e)}, extra=outbound_extra())
        return JSONResponse(content=False)

    logger.info({"event": "esb.upload.succeeded", "server_path": server_path, "server_file": server_file}, extra=outbound_extra())
    return JSONResponse(content=True)


@app.get("/health")
def health():
    return {"status": "ok", "service": "esb"}
...
def get_log_level():
    return {
        ...
        "effectiveLevels": {
            ...
            "esb": logging.getLevelName(logging.getLogger("esb").getEffectiveLevel()),
